...
function setEditorHashByPath(path: string | null, replace = false) {
...
function addCustomItem(key: string, value: string, afterAdd?: () => void) {
...
function loadUnsavedNewFileDraft(): UnsavedNewFileDraft | null {
	...
	try {
		...
		if (
			String(parsed.user) !== state.user ||
			String(parsed.repo) !== state.repo
		) {
			return null;
		}
		return {
...
function loadDraft(path: string, originalContent: ParsedContent) {
	try {
		...
		if (draft) {
			...
			const originalFull = createFullMarkdown(
			...
			if (
				confirm(
					`发现该文件于 ${savedDate} 保存的草稿。内容与远程版本不同，要加载草稿吗？`,
				)
			) {
				populateEditor(draft.metadata, draft.body);
				markAsDirty();
...
function findBestSourceSelection(source: string): {
	start: number;
	end: number;
} {
	...
	for (const needle of directNeedles) {
		...
		if (index !== -1) {
			const end =
				context.selectedText && needle === context.selectedText
					? index + needle.length
					: index;
...
function focusSourceSelection(start: number, end: number = start) {
	...
	requestAnimationFrame(() => {
		...
		const linesBefore =
			ui.sourceContent.value.slice(0, safeStart).split("\n").length - 1;
...
function openSourceModal(alignWithCursor = false) {
...
function resetEditorState(clearHash = true) {
...
async function confirmAndDeleteFile(path: string, sha: string | null) {
	...
	if (
		!confirm(`你确定要删除文件 "${path.split("/").pop()}" 吗？此操作无法撤销。`)
	)
		return;
...
async function handleDeleteButtonClick() {
	...
	await confirmAndDeleteFile(state.currentFile.path, state.currentFile.sha);
}
...
function handleCreateNewFile() {
	const filenameInput = ui.newFileInput.value.trim();
	if (!filenameInput) {
...
async function loadFileBySlug(slug: string) {
	...
	const message =
		lastError instanceof Error ? lastError.message : `未找到文章: ${slug}`;
