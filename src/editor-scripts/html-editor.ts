...
const ui = {
	...
	filenameInput: document.getElementById(
		"html-filename-input",
	) as HTMLInputElement,
	...
	ai: {
		...
		refreshModelsBtn: document.getElementById(
			"ai-refresh-models-btn",
		) as HTMLButtonElement,
...
function findFileByName(filename: string) {
	return (
		files.find((file) => file.name === normalizeHtmlFilename(filename)) ?? null
	);
}
...
async function loadFiles() {
	...
	files = (Array.isArray(data) ? data : [])
		.filter(
			(item) =>
				item.type === "file" &&
				String(item.name).toLowerCase().endsWith(".html"),
		)
		.map((item) => ({
...
function createNewFileTemplate(filename?: string, content = "") {
	const finalName = normalizeHtmlFilename(
...
async function saveCurrentFile() {
	...
	try {
		...
		if (isRename) {
			result =
				existingTarget && existingTarget.path !== state.currentFile.path
		...
		setCurrentFile(
			{ path: targetPath, sha: result.content.sha, isNew: false },
			filename,
		);
...
async function deleteCurrentFile() {
	...
	const yes = confirm(
		`确认删除 ${pathToFilename(state.currentFile.path)} 吗？`,
	);
	if (!yes) return;
	try {
		await deleteFile(state, state.currentFile.path, state.currentFile.sha);
...
function downloadCurrentFile() {
	const filename = normalizeHtmlFilename(
		ui.filenameInput.value || "untitled.html",
	);
	const blob = new Blob([getEditorValue()], {
...
async function autoCreateHtml(content: string, preferredName?: string) {
	const filename = ensureUniqueFilename(
		preferredName || `pasted-${Date.now()}.html`,
	);
	...
	try {
		...
		return {
			...
			models: Array.isArray(parsed.models)
				? parsed.models.map((model) => String(model))
				: [],
		};
	} catch {
...
function bindEvents() {
	...
	ui.fileList.addEventListener("click", (event) => {
		const target = event.target as HTMLElement | null;
		const fileButton = target?.closest(
			"[data-file-path]",
		) as HTMLElement | null;
