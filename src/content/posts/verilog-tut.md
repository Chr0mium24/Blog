...
### 2.运算符
  ...
  - **关系操作符**: >, <, >=, <=, == (等于), != (不等于)
  - **移位操作符**: << (左移), >> (右移)
  - **拼接操作符**: {}，例如 {a, b} 表示将 a 和 b 两个信号拼接成一个更宽的信号。
  - **复制操作符**: {{}}，例如 {4{1'b1}} 表示 4'b1111。
...
在现代数字电路设计中，设计师通常采用**自顶向下 (Top-down)** 的方法：
1.  首先用**行为级**模型来定义复杂模块的功能，确保算法正确。
2.  然后使用**数据流**和**结构级**的描述，将整个设计连接成一个完整的系统。
3.  最后，**综合工具 (Synthesis Tool)** 会自动将高层次的行为级和数据流描述转换为底层的、由逻辑门构成的**门级网表 (Gate-level Netlist)**，这本质上就是最底层的结构级描述。